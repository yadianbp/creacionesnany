...
class ThreadTestCase(TestCase):
    ...
    def test_filter_non_exists_thread(self):
        threads = Thread.objects.filter(users=self.user1).filter(users=self.user2)
        self.assertEqual(len(threads), 0)

    def test_add_messages_tothread(self):
        self.thread.users.add(self.user1,self.user2)
        message1 = Message.objects.create(user=self.user1, content='Buenos dias')
        message2 = Message.objects.create(user=self.user2, content='Como estas')
